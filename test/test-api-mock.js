...
const MockAPI = class MockApi {
    parseEndpoint(outParser, inParser = x => x) {
        return mwsApiName => async (callOptions, opt) => outParser(
            await this.callEndpoint(mwsApiName, inParser(callOptions), opt),
        );
    }
    ...
    callEndpoint(name/* , callOptions, opt */) {
        ...
        const mockData = fs.readFileSync(
            `${MOCK_DATA}/${endpoint.category}/${endpoint.action}Response.xml`,
        );

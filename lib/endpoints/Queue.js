...
class Queue {
    constructor({
        ...
        this.restoreRate = restoreRate || 1;
    ...
    set addToQueue(queueItem) {
        return queueItem instanceof Array
            ? this.queue.push(...queueItem)
            : this.queue.push(queueItem);
    }

    // getter to retrieve the qurrent items on the queue useful for testing purpose.
    get getQueue() {
        return this.queue;
    }
...
class QueueScheduler {
    ...
    runQueue(queueName) {
        return this.getQueue(queueName).runQueue();
    }

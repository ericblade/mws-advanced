...
const outputParser = (out) => {
    ...
    // to Array type.
    try {
        res.ShipmentEventList.ShipmentEvent.ShipmentItemList.ShipmentItem = forceArray(
...
const outParserSingle = (out) => {
    const { result } = outputParser(out);
    return result;
};

const listFinancialEvents = api => api.parseEndpoint(outParserSingle, inputParser)('ListFinancialEvents');

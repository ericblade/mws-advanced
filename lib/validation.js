// TODO: testing indicates some weirdness in this call. It throws for things that are outside
// of ranges, but otherwise valid, it throws for things that are not in a list of valid values,
// but it does not throw for basic type incompatibility, relying on it's caller to throw.
// Should this throw on all errors, or should it not throw on any errors? It's a lot easier
// to determine the *reason* for the throw here, so I lean towards fixing this to throw an error
// for all invalid things.
// For right now, I'm going to leave it exactly as is, because there's always a throw at the end
// user, but this makes unit testing weird.

// we expect xs:dateTime to come in as a ISO string. This is a little confusing because
// the consumer, validateAndTransform allows Date objects as input.
...
const isType = (type, test, definition) => {
    ...
    switch (type) {
        ...
        { // TODO: should we add validation that minValue and maxValue are within range as well? or just let consumers of API break it if they want.. :-)

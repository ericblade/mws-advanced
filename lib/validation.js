...
const isType = (type, test, definition) => {
    let valid = false;
    switch (type) {
        case 'xs:int': // positive or negative integer
        {
            const newTest = parseInt(test, 10);
            const minValue = (definition && definition.minValue) || -2147483648;
            const maxValue = (definition && definition.maxValue) || 2147483647;
            // eslint-disable-next-line eqeqeq
            valid = test == newTest && (!minValue || newTest >= minValue) && (!maxValue || newTest <= maxValue);
            if (!valid) {
                throw new Error(`Value ${test} expected xs:int`);
            }
            break;
        }

...
const reformatSummary = (summary) => {
    ...
    ret.lowestPrices = summary.LowestPrices.LowestPrice.map(lp => ({
    ...
    // console.warn('**** buyBoxPrices=', buyBoxPrices);
    ret.buyBoxPrices = buyBoxPrices && buyBoxPrices.map(bb => ({
 ...
 *
 * @param {object} options Options to pass to GetLowestPricedOffersForASIN
 * @param {string} options.MarketplaceId Marketplace ID
 * @param {string} options.ASIN ASIN
 * @param {string} options.ItemCondition New, Used, Collectible, Refurbished, Club
 *
...
const getLowestPricedOffersForASIN = async (options) => {
    ...
    ret.lowestOffers = offers.map(offer => ({

...
const reformatSummary = (summary) => {
    ...
    const numberOfOffers = forceArray(summary.NumberOfOffers.OfferCount);
    ret.numberOfOffers = numberOfOffers.map(reformatOfferCount);

    ret.listPrice = summary.ListPrice;
    const lowestPrices = forceArray(summary.LowestPrices.LowestPrice);
    ...
    const buyBoxEligibleOffers = forceArray(summary.BuyBoxEligibleOffers.OfferCount);

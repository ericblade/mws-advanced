...
function requestPromise(requestData, opt = {}) {
    return new Promise((function reqPromise(resolve, reject) { // eslint-disable-line prefer-arrow-callback
        ...
        mwsInterface.request(requestData, (err, result, headers) => {
            if (err) {
                reject({ error: err, headers });
            } else {
                // TODO: headers 'x-mws-quota-max', 'x-mws-quota-remaining', and
                // 'x-mws-quota-resetson' will be received HERE for throttle tracking

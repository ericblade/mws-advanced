...
// a simple cache to store the data from endpoints that appear to have identical calls
// works under assumption that the end user is going to be incorporating mws into some kind
// of long running process, so you should probably clear it at least once an hour, since API
// throttling also resets after an hour.  Also, your memory usage will probably grow wildly out
// of control if you don't clear it every now and then.
// There is not an auto-clearing mechanism here, as setting an interval or timeout will prevent
// the Node process from exiting.
// Call 'callEndpoint' with useCache: false to ignore caching.
// A smarter system might use https://www.npmjs.com/package/caching-map or
// https://github.com/ianp/es6-lru-cache or something else entirely to do the caching.

let callCache = {};
/**
 * Clear any data in the call cache. You should probably call this occasionally, if you are
 * running this in any kind of a server environment, where it will be running for long periods
 * of time, else your memory usage will grow out of control.
 *
 */
const clearCallCache = () => { callCache = {}; };
// TODO: devise some way of testing the cache, so we can validate automatically if useCache
// is working properly

...
const getProductCategoriesForAsins = ({ marketplaceId, asins }) => {
    const results = Promise.all(asins.map(asin => getProductCategoriesForAsin({ marketplaceId, asin })));
    return results;
};
...
const getProductCategoriesForSkus = ({ marketplaceId, skus }) => {
    const results = Promise.all(skus.map(sku => getProductCategoriesForSku({ marketplaceId, sku })));
    return results;
};
